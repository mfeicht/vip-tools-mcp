...
export function inspectRoutineMaterialCommentIdempotency({
  ...
  const priorMaterialStories = (stories || [])
  ...
  if (!priorMaterialStoryGids.includes(supersedesGid)) {
    return {
      status: "invalid_supersedes_story",
      allowed: false,
      prior_material_story_gids: priorMaterialStoryGids,
...
export function detectRoutineFollowUpSignals({ finalComment, completionBasis, followUpNotRequiredBasis }) {
  ...
  const noFollowUpClaim =
    /\bkein(?:e|er|en)?\s+(?:weitere\s+)?(?:aktive\s+)?(?:nacharbeit|folgeaufgabe|follow[\s-]?up|handoff|aktion)\s+(?:noetig|nötig|erforderlich|offen)\b/i.test(
      combinedText
    ) ||
  ...
  const existingTaskCoverageClaim =
    /\b(?:bestehende[nrs]?|vorhandene[nrs]?|naechste[nrs]?|nächste[nrs]?|kuenftige[nrs]?|künftige[nrs]?|zukuenftige[nrs]?|zukünftige[nrs]?)?\s*(?:routine|folgeaufgabe|follow[\s-]?up|task|aufgabe)\b/i.test(
      combinedText
    ) &&
    /\b(?:deckt|abgedeckt|uebernimmt|übernimmt|stellt\s+sicher|existiert|vorhanden|angelegt|eingeplant|terminiert|laeuft|läuft)\b/i.test(
      combinedText
    );
  return {
