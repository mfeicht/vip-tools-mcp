...
export async function fetchInstagramBusinessDiscoveryImage(
  ...
  await withImageFetchSlot(async () => {
    for (let attempt = 0; attempt < MAX_IMAGE_FETCH_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (attempt < MAX_IMAGE_FETCH_ATTEMPTS - 1 && isRetryableImageError(error)) {
          await wait(IMAGE_RETRY_DELAY_MS * (attempt + 1));
          continue;
        }
        const status = Number(error?.response?.status);
