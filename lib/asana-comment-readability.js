  ...
  for (const [sectionIndex, section] of sections.entries()) {
    ...
    for (const [paragraphIndex, paragraph] of (section?.paragraphs || []).entries()) {
      fields.push([
        ...
        typeof paragraph === "string" ? paragraph : (paragraph?.runs || []).map((run) => run.text).join("")
      ]);
    }
    for (const [bulletIndex, bullet] of (section?.bullets || []).entries()) {
      fields.push([`sections[${sectionIndex}].bullets[${bulletIndex}]`, bullet]);
    }
    for (const [itemIndex, item] of (section?.numbered || []).entries()) {
      fields.push([`sections[${sectionIndex}].numbered[${itemIndex}]`, item]);
    }
  }
