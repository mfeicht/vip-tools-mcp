...
export const DASHBOARD_REFRESH_POLL_MAX_AGE_MS = 5 * 60 * 1000;
export const DASHBOARD_CONTROL_POLL_MAX_AGE_MS = 5 * 60 * 1000;

export function dashboardRefreshPollMessage({ timestamp, nonce }) {
  return Buffer.from(`vip-dashboard-refresh-poll-v1\n${timestamp}\n${nonce}`, "utf8");
}

export function dashboardControlPollMessage({ timestamp, nonce }) {
  return Buffer.from(`vip-dashboard-control-poll-v1\n${timestamp}\n${nonce}`, "utf8");
}

function verifyDashboardSignedPoll({
  timestamp,
  nonce,
  signatureBase64,
  publicKeyPem,
  ...
  if (!timestamp || !nonce || !signatureBase64 || !publicKeyPem) {
    throw new Error("Dashboard poll timestamp, nonce, signature or public key is missing.");
  }
  ...
  if (
    ...
    throw new Error("Dashboard poll is outside the accepted time window.");
  }
  if (!/^[a-zA-Z0-9_-]{12,100}$/.test(String(nonce))) {
    throw new Error("Dashboard poll nonce is invalid.");
  }
  const signature = Buffer.from(signatureBase64, "base64");
  const verified = verify(null, message({ timestamp, nonce }), createPublicKey(publicKeyPem), signature);
  if (!verified) throw new Error("Dashboard poll signature is invalid.");
  return { timestamp, timestampMs, nonce };
}

export function verifyDashboardRefreshPoll({
  timestamp,
  nonce,
  signatureBase64,
  publicKeyPem,
  ...
  return verifyDashboardSignedPoll({
    timestamp,
    nonce,
    signatureBase64,
    publicKeyPem,
    now,
    maxAgeMs: DASHBOARD_REFRESH_POLL_MAX_AGE_MS,
    message: dashboardRefreshPollMessage
  });
}

export function verifyDashboardControlPoll({
  timestamp,
  nonce,
  signatureBase64,
  publicKeyPem,
  ...
  return verifyDashboardSignedPoll({
    ...
    publicKeyPem,
    now,
    maxAgeMs: DASHBOARD_CONTROL_POLL_MAX_AGE_MS,
    message: dashboardControlPollMessage
  });
}
