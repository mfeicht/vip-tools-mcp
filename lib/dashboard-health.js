...
export function verifyDashboardRefreshPoll({
  ...
  if (!timestamp || !nonce || !signatureBase64 || !publicKeyPem) {
    throw new Error("Refresh poll timestamp, nonce, signature or public key is missing.");
  }
  const timestampMs = Date.parse(timestamp);
  if (
    !Number.isFinite(timestampMs) ||
    timestampMs > now + 60_000 ||
    now - timestampMs > DASHBOARD_REFRESH_POLL_MAX_AGE_MS
  ) {
    throw new Error("Refresh poll is outside the accepted time window.");
  }
  if (!/^[a-zA-Z0-9_-]{12,100}$/.test(String(nonce))) {
    throw new Error("Refresh poll nonce is invalid.");
  }
  const signature = Buffer.from(signatureBase64, "base64");
  const verified = verify(
    null,
    dashboardRefreshPollMessage({ timestamp, nonce }),
    createPublicKey(publicKeyPem),
    signature
  );
  if (!verified) throw new Error("Refresh poll signature is invalid.");
